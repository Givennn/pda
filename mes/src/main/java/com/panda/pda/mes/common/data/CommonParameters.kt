...
//}
    ...
    fun pushParameters(data: Map<String, List<Parameter>>) {
        dictionary =
            data.filter {
                DataParamType.values().map { type -> type.name }.contains(it.key)
                    .also { isContains ->
                        if (!isContains) {
                            Timber.e("${DataParamType::class.java.simpleName} can not find element: ${it.key}")
                        }
                    }
            }
                .mapKeys { DataParamType.valueOf(it.key) }
                .toMutableMap()
...
enum class DataParamType {
    ...
    QMS_ADVERSE_DEAL_TYPE,
    QMS_ADVERSE_DEAL_STATUS,
    MESSAGE_LEVEL
}