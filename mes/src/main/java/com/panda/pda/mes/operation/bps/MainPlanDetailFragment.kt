 ...
 */
class MainPlanDetailFragment: BaseFragment(R.layout.fragment_quality_detail) {
    private val viewBinding by viewBinding<FragmentQualityDetailBinding>()

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        viewBinding.topAppBar.setNavigationOnClickListener { navBackListener.invoke(it) }
        viewBinding.topAppBar.title = getString(R.string.main_plan_detail)
        val recordList = arguments?.getGenericObjectString<DataListNode<CommonOperationRecordModel>>(
            ...
            viewPage.adapter = object : FragmentStateAdapter(parentFragmentManager, lifecycle) {
                ...
                override fun createFragment(position: Int): Fragment {
                    return when (position) {
                        0 -> MainPlanDetailInfoFragment(detail)
