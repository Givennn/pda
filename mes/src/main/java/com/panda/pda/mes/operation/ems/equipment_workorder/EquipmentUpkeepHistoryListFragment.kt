...
/**
 * 保养记录,,传入设备或者模具id，传入设备或者模具类型，1：设备，2：模具
 */
class EquipmentUpkeepHistoryListFragment(
    private var facilityId: String = "1",
    private var facilityType: String = "1",
) :
            ...
            override fun onLoadMore(page: Int, totalItemsCount: Int, view: RecyclerView?) {
                if (facilityType == "1") {
                    WebClient.request(EquipmentApi::class.java)
                        .pdaEmsEquipmentMaintainOrderListGet(MesStringUtils.stringToInt(facilityId),
                            10,
                            page
                        )
                        .bindToFragment()
                    ...
                    WebClient.request(EquipmentApi::class.java)
                        .pdaEmsModuleMaintainOrderListGet(MesStringUtils.stringToInt(facilityId),
                            10,
                            page
                        )
                        .bindToFragment()
    ...
    fun api(
        ...
        if (facilityType == "1") {
            ...
            return WebClient.request(EquipmentApi::class.java)
                .pdaEmsEquipmentMaintainOrderListGet(facilityId)
                .doFinally { scrollListener.resetState() }
        } else {
            //模具查询维修记录
            return WebClient.request(EquipmentApi::class.java)
                .pdaEmsModuleMaintainOrderListGet(facilityId)
                .doFinally { scrollListener.resetState() }
        }
    }
